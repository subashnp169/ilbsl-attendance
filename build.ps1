...
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;background:var(--paper);color:var(--ink)}
.topbar{background:linear-gradient(135deg,var(--navy),var(--navy-2));color:#fff;padding:18px 20px 22px;text-align:center;position:sticky;top:0;z-index:100}
...
.tabs{display:flex;background:var(--navy);overflow-x:auto}
.tabs button{flex:1;min-width:0;padding:12px 6px;border:none;background:transparent;color:rgba(255,255,255,.55);font-size:12px;font-weight:600;cursor:pointer;border-bottom:3px solid transparent;white-space:nowrap}
.tabs button.active{color:#fff;border-bottom-color:var(--gold);background:rgba(255,255,255,.07)}
.tab-content{display:none;max-width:680px;margin:0 auto;padding:16px}
...
@media print{.no-print,.topbar,.tabs,.rc .pb,.rc .rb{display:none !important}body{background:#fff}.tab-content{display:block !important;padding:0;max-width:100%}.card{border:none;padding:8px}.rt{font-size:9px}.rt th{background:#333 !important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.st{width:60px;height:30px}.sb,.dh{-webkit-print-color-adjust:exact;print-color-adjust:exact}@page{margin:.5cm;size:landscape}}
</style>
</head>
<body>
<div class="topbar">
...
let attCurrentDay = "Day 1";
let fbCurrentDay = "Day 1";
let rptData = [];
...
if(!isUrlConfigured()){showMsg("attMsg","Setup incomplete: configure SCRIPT_URL in the source code.","err");return;}
...
if(!isUrlConfigured()){showMsg("fbMsg","Setup incomplete: configure SCRIPT_URL.","err");return;}
...
function loadReport(){
if(!isUrlConfigured()){
document.getElementById("rptBody").innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:#b3261e">Configure SCRIPT_URL first.</td></tr>';
return;
}
...
renderReportStats && updateReportStats();
renderReportTable();
});
